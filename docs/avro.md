...
Эндпоинт работает в единственном режиме — Avro (Confluent Schema Registry 5.3.8). Локальные `.avsc` обязательны: они используются как источник Phoenix-метаданных, подсказок по CF/PK и как кеш перед регистрацией схем в SR. Других режимов/форматов нет.
...
Схемы описывают **структуру Avro‑записи**, которую собирает `PayloadBuilder`. Для корректной работы добавьте:

- На корневом уровне:
  - `"h2k.pk"` — массив имён PK в порядке Phoenix-таблицы.
  - `"h2k.saltBytes"` — длина соли rowkey (0..8).
  - `"h2k.capacityHint"` — ожидаемое число заполненных полей.
- Для каждой колонки:
...
## 2.1 Детерминированный порядок полей

- Порядок полей `record` строго равен порядку `fields` в `.avsc`.
- Для `map` ключи сортируются по возрастанию, чтобы байтовый payload был стабильным.
- Для `array` порядок элементов сохраняется как в источнике (без сортировки).

> Рекомендация: не полагайтесь на порядок ключей входных `Map` — он нормализуется сортировкой.

### Производительность map

- Сортировка ключей добавляет O(n log n) на каждую map‑коллекцию.
- Если map‑полей нет в схемах — накладных расходов нет.
- Если map‑поля есть и они большие (сотни/тысячи ключей), учитывайте сортировку при оценке QPS.
...
Новая инсталляция сразу настраивает Schema Registry и следит за `h2k.capacityHint` в `.avsc`.
