...
7. **Диагностика**
   - При ошибках регистрации в логах появляются WARN с адресом SR и телом ответа.
   - DEBUG-логгер `kz.qazmarka.h2k.schema.registry.avro.phoenix.AvroPhoenixSchemaRegistry` фиксирует, из какого `.avsc`
     считана схема (путь, количество полей, PK, `h2k.saltBytes`, `h2k.capacityHint`). Это помогает убедиться, что
     локальные файлы подтянуты.
   - DEBUG-логгер `kz.qazmarka.h2k.payload.serializer.avro.ConfluentAvroPayloadSerializer` выводит сравнение локальной
     схемы с последней версией в Schema Registry (fingerprint, версия, id) и отмечает первую успешную/неуспешную
     регистрацию. Удобно при обновлении `.avsc`.
