...
- Эффективность фильтра отслеживается автоматически: после ≈500 строк эндпоинт пишет `INFO` с долей отфильтрованных строк. Если доля < 1% — появится `WARN` с рекомендацией убрать `h2k.cf.list`.
...
### 4. Метрики автотюнера BatchSender

- `producer_batch_fail_streak_current` — текущая непрерывная серия «тихих» неудач `tryFlush`. Рост > 5 — Kafka не подтверждает записи, awaitEvery принудительно держится на минимуме.
- `producer_batch_fail_streak_max` — максимальная серия с момента старта. Используйте для алерта (например, > 10).
- `producer_batch_fail_last_ms` — отметка времени последней неудачи. Если значение давно не обновлялось, серия завершена.
- `producer_batch_fail_last_await` — значение `awaitEvery`, на котором произошло последнее падение.
- `producer_batch_autotune_decisions_total` / `producer_batch_autotune_last.await` — контроль того, что тюнер продолжает принимать решения.

**Рекомендации:**

1. Настройте алерт на `producer_batch_fail_streak_max >= 10` или «обновление `producer_batch_fail_last_ms` чаще одного раза в минуту» — это признак деградации Kafka (таймауты ACK).
2. При ручной диагностике снимайте значения `awaitEvery` до/после: тюнер должен опуститься до минимума и после стабилизации вернуться к базовым значениям (видно по `producer_batch_autotune_last.await`).
3. Если серия таймаутов продолжается и автотюнер не помогает, переключите peer в профиль RELIABLE или уменьшите `h2k.producer.await.every` вручную и сообщите команде Kafka.
