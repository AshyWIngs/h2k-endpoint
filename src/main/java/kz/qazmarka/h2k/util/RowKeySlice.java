...
public class RowKeySlice implements Comparable<RowKeySlice> {
    ...
    protected final void resetState(byte[] array, int offset, int length) {
        Objects.requireNonNull(array, "Аргумент 'array' (исходный байтовый массив) не может быть null");
    ...
    public int compareTo(RowKeySlice other) {
        if (Objects.isNull(other)) return 1; // по контракту Comparable: любой non-null > null
